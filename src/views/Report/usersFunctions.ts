...
import type { GameRecordItem } from "@/api/reportApi";

export const departOptions = ref<DepartmentNode[]>([]);
export const datas = ref<GameRecordItem[]>([]); // 数据数组
...
export const pageIndex = ref<number>(1); //当前页
...
export const handleQuery = async (filters: { name: string }) => {
  ...
  const para = ref<UserRequest>({
    ...
    f: "0",
    key: filters.name,
    beginTime: "2024/12/10 00:00:00",
    endTime: "2024/12/10 23:59:59",
    platform_category: '0_0'
  });
...
export const addSubmit = async () => {
  ...
  await formEl.validate(async (isValid) => {
    if (isValid) {
      ...
      postData.birth =
        !postData.birth || postData.birth == ""
          ? formatDate(new Date(), "yyyy-MM-dd")
          : formatDate(new Date(postData.birth), "yyyy-MM-dd");
