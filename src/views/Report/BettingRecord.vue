<template>
    <section>
        ...
        <!-- 列表 -->
        <el-table :data="datas" ref="tableRef" v-loading="listLoading" @select="dialogCheck"
            @row-click="selectCurrentRow" row-key="Id" border style="width: 100%">
            <el-table-column type="selection" width="50"> </el-table-column>
            <el-table-column type="index" width="80"> </el-table-column>
            <el-table-column prop="Status" label="Status" width="" sortable>
            </el-table-column>
            <el-table-column prop="GameLocalTime" label="Betting Time" width="" sortable>
            </el-table-column>
            <el-table-column prop="ResultTime" label="Settlement time" width="" sortable>
                <template #default="{ row }">
                    <el-tag v-for="item in row.RoleNames" :key="item.Id">{{ item }}</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="Acnt" label="Username" width="" sortable>
            </el-table-column>
            <el-table-column prop="Branch" label="Branch" width="" :formatter="formatSex" sortable>
            </el-table-column>
            <el-table-column prop="BetID" label="Note number" width="" sortable>
            </el-table-column>
            <el-table-column prop="GamePlatform" label="Game Provider" width="" sortable>
            </el-table-column>
            <el-table-column prop="GameCategoryName" label="Category" width="" sortable>
            </el-table-column>
            <el-table-column prop="GameName" label="Game" width="" sortable>
            </el-table-column>
            <el-table-column prop="BetAmount" label="Turnovers" width="" sortable>
            </el-table-column>
            <el-table-column prop="NetWin" label="Win/Loss" width="" sortable>
            </el-table-column>
            <el-table-column prop="BetAmount" label="Effective Turnovers" width="" sortable>
            </el-table-column>
            <el-table-column prop="BetID" label="Note number" width="" sortable>
            </el-table-column>
            <el-table-column prop="Payout" label="Payout" width="" sortable>
                <template #default="{ row }">
        ...
        <el-col :span="24" class="toolbar">
            <el-pagination layout="total, prev, pager, next, jumper" @current-change="handleCurrentChange"
                :page-size="pageSize" :total="total" :page-sizes="[10, 20, 50, 100]" :current-page="pageIndex"
                style="float: right">
            </el-pagination>
        </el-col>
...
import { getGameRecordApi, type GameRecordItem, type UserRequest } from '@/api/reportApi';

// 从 userFunctions.ts 导入
import {
    handleQuery, handleAdd, handleEdit, handleDel, datas, listLoading, isResouceShow,
    pageIndex, pageSize, total, addLoading, addFormRef, addSubmit, addFormVisible, addForm, departOptions, roleOptions,
...
const formatSex = (row: GameRecordItem) => {
    return row.sex === 1 ? "男" : row.sex === 0 ? "女" : "未知";
};

const formatBirth = (row: GameRecordItem) => {
...
// 选中当前行
const dialogCheck = async (selection: GameRecordItem[], row?: GameRecordItem) => {
...
const selectCurrentRow = (val: GameRecordItem) => {
    if (!val) return;
...
const handleCurrentChange = async (val: number) => {
    ...
    try {
        const { response } = await getGameRecordApi(para.value);
